...
class PermissionsController extends Controller
{
    ...
    public function fetchpermissionsdata()
    {
        if(request()->ajax()) {
            ...
            $response = array();
            $Filterdata = Permission::select('*')->orderBy('id','asc')->get();
            if(!empty($Filterdata))
            {
                $i = 0;
...
//                        $response[$i]['action'] = "";
//                        $response[$i]['action'] = '<a href="javascript:void(0)" class="btn editPermission" data-id="'. $permission->id .'"><i class="fa fa-edit  text-dark"></i></a>';
                    }
                    else
                    {
                        /*if(!auth()->user()->hasRole('superadmin'))
